...
In this section, we cover the cases where the pod or at least the VM have started, but the container fails to start.
...
Inspect the INITDATA field in the  xref:02-configure-osc.adoc#pp-cm[peer-pods-cm] configmap in OSC namespace, or alternatively if you are providing it manually as explained xref:03-deployment-settings.adoc#initdata-ann[here]:
...
* Make sure `image_security_policy_uri` points to an existing policy, and the policy points to an existing secret
...
After ANY change to any config/secret of the Red Hat build of Trustee operator, remember to xref:02-configure-trustee.adoc#trustee-restart[restart the deployment].
...
If you have access to the Red Hat workspace on Slack, feel free to post the error on `#forum-sandboxed-containers`! If you don't, contact a Red Hat representative and forward your question to them.
